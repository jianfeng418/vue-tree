...
<script>
	...
	export default{
		...
		components:{'v-tree-item':{
			name:'v-tree-item',
			template:'<ul class="cus_tree_ul" v-if="treeData && treeData.length" :class="{cus_tree_ulLine:(treeData && treeData.length)}"><li v-for="item in treeData"><div class="cus_item_content" @click="clickNode(item)" @click.cus="clickFun(JSON.stringify(item))" :id= item.id   :class="{active:item.active}">'+
